...
{
	//pair<int, int> directions[4]={make_pair(1,0),make_pair(-1,0),make_pair(0,1),make_pair(0,-1)};
...
// cout << "adding test " << current_coor.first <<" " << current_coor.second << " "<< current_char 
// <<" " << coor.first << " " << coor.second << " " << neighbour_char <<" "<< (current_char + 1 >= neighbour_char) << endl;
			check_list.push_back(coor);
		}
...
void print_queue(deque<data_t> to_examine){
	for(int i = 0; i < to_examine.size(); ++i){
		cout << "[" << to_examine[i].coor.first << "," << to_examine[i].coor.second << "] ";
	}
	cout << endl;
}

int bfs(pair<int, int> start, int width, int height, vector<string> lines){
	deque<data_t> to_examine;
	vector<pair<int,int> > alreadychecked;
	bool found = false;

	data_t startpoint {.coor=start,.steps = 0};
	to_examine.push_back(startpoint);
	...
	for(int y = 0; y < height; ++y){
		for(int x = 0; x < width; ++x){
			if(lines[y][x] == 'a'){
				...
				int steps = bfs(start, width, height, lines);
				if (steps < min_steps && steps != -1)
					min_steps = steps;
			}
		}
	}
