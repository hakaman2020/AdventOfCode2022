...
void init_monkey(monkey_t & monkey, vector<string> lines, int i){
	...
	for(int j = 2; j < split.size(); j++){
		monkey.items.push_back(stol(split[j]));
	}
...
void	process_monkey(monkey_t monkeys[], int amount_of_monkeys, monkey_t & monkey, int gcd){
...
int main(int argc, char ** argv){
	...
	for(int round = 1; round <= amount_of_rounds; round++){
		...
		for(int i = 0; i < amount_monkeys; ++i){
			process_monkey(monkeys, amount_monkeys, monkeys[i], gcd);
		}
		for(int i = 0; i < amount_monkeys; ++i){
			//print_monkey(monkeys[i], SHORT);
			cout << "Monkey "<< i << " " << monkeys[i].items_inspected << endl;
		}
	}

	unsigned long highest[2]={0,0};
	for(int i = 0; i < amount_monkeys; ++i){
